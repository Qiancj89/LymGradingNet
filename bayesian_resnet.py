...
class BayesianResNet():
    ...
    def __init__(self, input_shape, num_classes, num_filters = 32, kernel = 3, blocks = [3, 4, 6, 3],
                 load = False, directory = './classification_model_ResNet34/batch_size32/'):
        ''''''
        ...
        if load: #load the weights from h5 file and set current epoch
            self.model.load_weights(directory + 'bayesian34_subject30_loss.h5', by_name = True)
            self.current_epoch = np.genfromtxt(directory + 'bayesian34_hist_subject30_loss.csv', delimiter = ',', skip_header = 0).shape[0] - 1
    ...
    def fit(self, X, Y, validation_data, epochs = 10, batch_size = 32, optimizer = 'adam',
        ...
        if save: # set callback functions if saving model
            ...
            mpath = directory + 'bayesian18_DDcGAN_subject30_loss.h5'
            hpath = directory + 'bayesian18_hist_DDcGAN_subject30_loss.csv'
