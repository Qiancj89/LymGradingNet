...
def train_models(resnet, bayesian_resnet, center, modality):
    ...
    resnet.fit(np.expand_dims(train, axis = -1), to_categorical(train_labels),
               ...
               epochs = 250, batch_size = 16, optimizer = opt1, save = True) #train model
    
    bayesian_resnet.fit(np.expand_dims(train, axis = -1), to_categorical(train_labels),
                        validation_data = (np.expand_dims(val, axis = -1), to_categorical(val_labels)),
                        epochs = 640, batch_size = 16, optimizer = opt2, save = True) #train model
...
def train_test_group(center, modality):
    ...
    train_label = zero_train_label+one_train_label+two_train_label
    train_label = np.array(train_label, dtype='int')
    train_patient_name = zero_train_patient_name+one_train_patient_name+two_train_patient_name
    train_patient_name = np.array(train_patient_name, dtype='<U32')
    train_patient_path = zero_train_patient_path+one_train_patient_path+two_train_patient_path
    train_patient_path = np.array(train_patient_path, dtype='<U32')
    test_label = zero_test_label+one_test_label+two_test_label
    test_label = np.array(test_label, dtype='int')
    test_patient_name = zero_test_patient_name+one_test_patient_name+two_test_patient_name
    test_patient_name = np.array(test_patient_name, dtype='<U32')
    test_patient_path = zero_test_patient_path+one_test_patient_path+two_test_patient_path
    ...
    with open('../../../lvpao/fusion_result2/three_class_train_'+modality+'_subject30_random2023.csv','w',newline='') as fi:
    ...
    with open('../../../lvpao/fusion_result2/three_class_test_'+modality+'_subject30_random2023.csv','w',newline='') as fi:
        writer = csv.writer(fi)
        writer.writerow(fname)
