...
with tf.Session() as sess:
    ...
    for c in range(0, len(center)):
        patient_name = os.listdir('F:/lvpao/fusion_result2/'+center[c]+'/')
        for p in range(0, len(patient_name)):
            patient_path = 'F:/lvpao/fusion_result2/'+center[c]+'/'+patient_name[p]
            data_gfp, data_pci = source_imgs_generate(patient_path)
            FUSION_IMG = np.zeros((len(data_gfp),180,180),dtype='float32')
            for i in range(0, len(data_gfp)):
                ...
                result_ = cv2.cvtColor(np.uint8(result_rgb), cv2.COLOR_RGB2GRAY)
                FUSION_IMG[i,:,:] = result_
            ...
            sitk.WriteImage(FUSION_z_sitk, savepath+center[c]+'/'+patient_name[p]+'/DPCN_Fusion.nii.gz')
            
            print('F:/lvpao/fusion_result2/'+center[c]+'/'+patient_name[p])