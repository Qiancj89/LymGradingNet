...
class ResNet():
    ...
    def __init__(self, load = False, directory = './classification_model_ResNet152/batch_size8/', input_shape = None, num_classes = None,
                 num_filters = 32, kernel = 3, blocks = [3, 8, 36, 3]):
        ''''''
        if load:
            self.model = load_model(directory + 'resnet152_subject30_loss.h5')
            self.current_epoch = np.genfromtxt(directory + 'resnet152_hist_subject30_loss.csv', delimiter = ',', skip_header = 0).shape[0] - 1
            self.model.summary()
        else:
    ...
    def build_model(self, input_shape, num_classes, num_filters, kernel, blocks):
        ...
        def residual_block(x, num_filters, kernel, stride, index):
            out = Conv2D(num_filters, 1, stride, padding = 'same', name = 'C{}'.format(index))(x)
            index += 1
            ...
            out = ReLU(name = 'RELU{}'.format(index-1))(out)
            out = BatchNormalization(name = 'BN{}'.format(index-1))(out)
            out = Conv2D(4*num_filters, 1, 1, padding = 'same', name = 'C{}'.format(index))(out)
            index += 1
            if stride != 1 or 4*num_filters != 64: #Need to downsample input by half to match dimensions
                x = Conv2D(4*num_filters, 1, stride, padding = 'same', name = 'C_DS{}'.format(index-4))(x)
    ...
    def fit(self, X, Y, validation_data = None, epochs = 10, batch_size = 32, optimizer = 'adam',
        ...
        if save: # set callback functions if saving model
            ...
            mpath = directory + 'resnet50_subject30_loss.h5'
            hpath = directory + 'resnet50_hist_subject30_loss.csv'
