...
def main():
    ...
    modality = 'FUSION_10_0'
    test_patient_name = []
    test_label = []
    test_patient_path = []
    f = open('../../../lvpao/fusion_result2/test_'+modality+'_subject30_random2023.csv','r')
    reader = csv.reader(f)
    for item in reader:
    ...
    fname = ['patient_name', 'label', 'center', 'pred_label', 'class1', 'class2', 'class3', 'class4']
    ...
    with open('classification_model_'+modality+'/batch_size32/test_subject30_'+modality+'_rprediction_micro.csv','w',newline='') as fi:
        writer = csv.writer(fi)
        writer.writerow(fname)
        for l in range(0, len(test_label)):
            writer.writerow(t3[l,:])
    fi.close()
    with open('classification_model_'+modality+'/batch_size32/test_subject30_'+modality+'_bprediction_micro.csv','w',newline='') as fi:
        writer = csv.writer(fi)
        writer.writerow(fname)
        for l in range(0, len(test_label)):
